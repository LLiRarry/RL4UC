...
class UC_Env(gym.Env):
    ...
    def __init__(self, gen_info, mode='train'):
        ...
        self.action_space = spaces.Box(low=-0.2, high=1, shape=(8,))
    ...
    def step(self, action):
        ...
        for i in range(action.shape[0]):
            if action[i]<0:
                actions_discrete.append(0)
                actions_continuous.append(0)
            else:
                actions_discrete.append(1)
                actions_continuous.append(action[i])
        ...
        # print(actions_discrete,"离散开停动作")
        # print(power_output,'功率输出')
